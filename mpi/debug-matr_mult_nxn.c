...
int main(int argc, char **argv) {
    ...
    printf("\nProsessi %d sai seuraavat alkiot:\n A = \n", id);
    for (i = 0; i < RC_PER_BLOCK*n; i++) {
        printf("%5.4f ", A_local[i]);
    }
    printf("\n B = \n");
    for (i = 0; i < RC_PER_BLOCK*n; i++) {
        printf("%5.4f ", B_local[i]);
    }
    printf("\n");
    ...
    printf("\nProsessi %d palautti seuraavat alkiot:\n", id);
    for (i = 0; i < RC_PER_BLOCK*RC_PER_BLOCK; i++) {
        printf("%5.4f ", C_local[i]);
    }
    printf("\n");
    ...
    if (id == ROOT) {
        ...
        for (i = 0; i < n; i++) {
            for (j = 0; j < n; j++) {
                // Pitää muistaa, että kukin prosessi on palauttanut
                // laskemansa lohkon rivimuodossa.
                // Rivin i sarakkeen j alkio löytyy taulukon AB
                // paikasta ind.
                //ind = i/2*8 + j/2*4 + i%2*2 + j%2;
                ind = (i/RC_PER_BLOCK) * RC_PER_BLOCK*n
                    + (j/RC_PER_BLOCK) * RC_PER_BLOCK*RC_PER_BLOCK
                    + (i%RC_PER_BLOCK) * RC_PER_BLOCK
                    + j%RC_PER_BLOCK;
                printf("%5.4f ", C[ind]);
            }
            printf("\n");
